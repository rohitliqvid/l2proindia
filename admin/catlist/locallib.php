...
function scorm_get_sco($id,$what=SCO_ALL) {
    if ($sco = get_record('tls_scorm_sco','id',$id)) {
       /* $sco = ($what == SCO_DATA) ? new stdClass() : $sco;
        if (($what != SCO_ONLY) && ($scodatas = get_records('tls_scorm_sco','scoid',$id))) {
            foreach ($scodatas as $scodata) {
                $sco->{$scodata->name} = $scodata->value;
            }
        } else if (($what != SCO_ONLY) && (!($scodatas = get_records('tls_scorm_sco','scoid',$id)))) {
            $sco->parameters = '';
        }
        return $sco;
    } else {
...
function scorm_insert_track($userid,$scormid,$scoid,$element,$value) {
    $id = null;
	$sql="select id from tls_scorm_sco_tracking where userid='$userid' AND scormid='$scormid' AND scoid='$scoid' AND element='$element'";
	$record = mysql_query ($sql); 
	$num=mysql_numrows($record);

    if ($num) {
        $track->value = $value;
        $track->timemodified = time();
		$curTime=time();
		$sqlupdate="update tls_scorm_sco_tracking set value='$value',  timemodified='$curTime' where userid='$userid' AND scormid='$scormid' AND scoid='$scoid' AND element='$element'";
		$recordupdate = mysql_query ($sqlupdate); 
        //$id = update_record('scorm_scoes_track',$track);
    } else {
        $track->userid = $userid;
        $track->scormid = $scormid;
        ...
        $track->timemodified = time();
		$curTime=time();
        $sqlInsert="insert into tls_scorm_sco_tracking (userid,scormid,scoid,element,value,timemodified) values('$userid','$scormid','$scormid','$element','$value','$curTime')";
		//echo $sqlInsert;
		//exit;
		$recordInsert = mysql_query ($sqlInsert); 
		//$id = insert_record('scorm_scoes_track',$track);
    }
	updateStatus($userid,$scormid,$scoid);
...
function updateStatus($vuserid,$vscormid,$vscoid)
{
			...
			if($num)
			{
				$uScoreValue=mysql_result($result,0,"value");
				if($uScoreValue >= 80)
				{

				$sql="update tls_scorm_sco_tracking set value='passed' where userid='$vuserid' AND scormid='$vscormid' AND scoid='$vscoid' AND element='cmi.core.lesson_status'";
				mysql_query($sql);
...
/// Gets all tracks of specified sco and user
	...
	*/
	$query = "SELECT * from tls_scorm_sco_tracking where userid=$userid AND scoid=$scoid order by element ASC";
	$tracks = mysql_query ($query); 
	$num=mysql_numrows($tracks);

   // if ($tracks = get_records_select('tls_scorm_sco_tracking',"userid=$userid AND scoid=$scoid",'element ASC')) {
	    if ($num) {
		...
		$row = mysql_fetch_assoc($tracks);
		$rid = $row['id'];
		$element=mysql_result($tracks,$j,"element");
		$value=mysql_result($tracks,$j,"value");
		
		if($element=='cmi.core.lesson_status')
		{
			$usertrack->{'cmi.core.lesson_status'}=$value;
			
		}

		if($element=='cmi.core.lesson_location')
		{
			$usertrack->{'cmi.core.lesson_location'}=$value;
			
		}

		if($element=='cmi.core.score.raw')
		{
			$usertrack->{'cmi.core.score.raw'}=$value;
			
		}

		if($element=='cmi.score.raw')
		{
			$usertrack->{'cmi.score.raw'}=$value;
			
		}

		if($element=='cmi.core.session_time')
		{
			$usertrack->{'cmi.core.session_time'}=$value;
			
		}

		if($element=='cmi.core.total_time')
		{
			$usertrack->{'cmi.core.total_time'}=$value;
			
		}

		if($element=='cmi.total_time')
		{
			$usertrack->{'cmi.total_time'}=$value;
			
		}

		if($element=='cmi.completion_status')
		{
			$usertrack->{'cmi.completion_status'}=$value;
