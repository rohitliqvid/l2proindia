...
function liqChart2LineGraph(vals1,vals2,chartLabel1,chartLabel2,pointLabel,container) {
	...
	for(i=0;i<=vals1.length;i=i+0.1) {
		d3.push(s1(i));//alert(s1(i));alert(s2(i));
		d4.push(s2(i));
	}
...
function liqChart2LineGraph2(vals1,vals2,chartLabel1,chartLabel2,pointLabel,container) {
	var todayDt= (new Date()).getTime();//alert(vals1.length);
	var d1 = [];
	var d2 = [];
	var startDt = todayDt - 24*60*60*1000*vals1.length*7;
	var sum =0;
	for(i=1;i<=vals1.length;i++) {
		var arr = new Array();
		arr[0] =startDt+24*60*60*1000*i*7;	
		arr[1] =vals1[i-1];
		d1.push(arr);
		var arr2 = new Array();
		arr2[0] =startDt+24*60*60*1000*i*7;	
		arr2[1] =vals2[i-1];
		d2.push(arr2);
	}
//	alert(d2);
	...
	//var startdate = new Date(d4[0][0]);
    //var startweek = startdate.getWeek();
    //alert(startweek);
	//return Math.ceil((((this - onejan) / 86400000) + onejan.getDay() + 1) / 7)
  //  var data = [[[1301634000000, 1], [1302238800000, 2], [1302843600000, 5]]];
    var ticks = [];
    var tmpweek = 1;
    for (var i = 0; i < d2.length; i++) {
      	//alert(tmpweek);exit;
      ticks.push([d2[i][0],"week"+tmpweek]); //corrected the missing closing bracket here..
      tmpweek = tmpweek +1;
    }//alert(ticks);
  ...
  $(container).length && $.plot($(container), [{
	  ...
	  //lines : {show:true, fill:true, lineWidth:0},
	 // lines : {show:true}
 	 //lines : {show:false, fill:false, lineWidth:0},	
	  points : {show: 20, radius: 2},
	  color :colorGreen
      }, {
          data: d4,
	  color : colorRed,
	...
	//},
        ...
        colors: ["#00B6D6"],
        xaxis: { mode: "time",ticks: ticks,//minTickSize: [1, "week"],
		//tickLength: 0
        	 axisLabel: 'X - week' +'&nbsp;&nbsp;&nbsp;'+' Y - score'
        },
        yaxis: {
          ticks: (function (axis) {
		var res =[];
		var space = axis.max - axis.min;//alert(axis.max);alert(axis.min);
		space /=5;
		var steps = axis.max/space;
		for(var i=0;i< steps+1;i++) { res[i] = space*i;}
		return res;
        }) },
	legend : {
		backgroundOpacity : 1,
