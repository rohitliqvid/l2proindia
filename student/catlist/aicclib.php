...
function scorm_parse_aicc($pkgdir,$scrCourseId,$scrName,$scrSummary,$scrFileName,$width,$height) {
    $version = 'AICC';
	$pkgtype='AICC';
	$maxgrade=100;
	$grademethod=1;
...
$sql="insert into tls_scorm (`course`,`name`,`reference`,`version`,`maxgrade`,`grademethod`,`launch`,`summary`,`browsemode`,`auto`,`width`,`height`) VALUES ('".$scrCourseId."','".$scrName."','".$scrFileName."','".$version."','".$maxgrade."','".$grademethod."','".$launch."','".$scrSummary."','".$browsemode."','".$auto."','".$width."','".$height."')";
	...
	$timelimitaction="";
	$datafromlms="";
	$masteryscore=$scodata->value;
	$next=0;
	$previous=0;
	
	$sql="insert into tls_scorm_sco (`scorm`,`manifest`,`organization`,`parent`,`identifier`,`launch`,`parameters`,`scormtype`,`title`,`prerequisites`,`maxtimeallowed`,`timelimitaction`,`datafromlms`,`masteryscore`,`next`,`previous`) VALUES ('".$scrId."','".$manifest."','".$organization."','".$parent."','".$identifier."','".$launch."','".$parameters."','".$scormtype."','".$title."','".$prequisites."','".$maxtimeallowed."','".$timelimitaction."','".$datafromlms."','".$masteryscore."','".$next."','".$previous."')";
	mysql_query($sql);
    return $launch;
}
