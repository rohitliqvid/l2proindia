...
function finish() {
	...
	if (API != null && LMSFinishCalled == false) {
		...
		//var lastScore=get_val("cmi.core.score.raw");
		//alert("last_score on finish:"+last_score)
		if($nTotalAssessmentPages!=0)
		{
		FinalScore=parseInt(($nCurrScore/$nTotalAssessmentPages)*100);
		}
		else
		{
		FinalScore=0;
		...
		//alert("last_score="+last_score);
	
		if((FinalScore > last_score) && $assessmentTopicId!=0)
		{
		set_val("cmi.core.score.raw", FinalScore);
		}
		else
		{
		set_val("cmi.core.score.raw", last_score);
		}
