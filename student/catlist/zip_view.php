...
$tresult = mysql_query ("SELECT * FROM tbl_user_course_pref WHERE user_id='$user_rowid' AND course_id='$docid'"); 
$tnum=mysql_numrows($tresult);

if($tnum)
{
$win_width=mysql_result($tresult,0,"win_width");
$win_height=mysql_result($tresult,0,"win_height");
$win_resize=mysql_result($tresult,0,"win_resize");
$win_scroll=mysql_result($tresult,0,"win_scroll");
$win_directory=mysql_result($tresult,0,"win_directory");
$win_location=mysql_result($tresult,0,"win_location");
$win_menu=mysql_result($tresult,0,"win_menu");
$win_tool=mysql_result($tresult,0,"win_tool");
$win_status=mysql_result($tresult,0,"win_status");
}
else
{
...
{
?>
<table width="784" cellspacing="2" cellpadding="3">
...
?>
 <table width="784" border="0" cellspacing="2" cellpadding="3">
  
	<tr height='40'><td colspan='2' valign='middle' class="content">This course has following lauch files. Click on a launch link to launch the course.</td></tr>
...
$launchfile=mysql_result($lresult,$i,"file_launch");
	?>
		...
		<tr> 
          ...
          <td valign='top' class="content"><a onFocus='this.blur()' onMouseOver='return showStatus();' href="javascript:openFile(<?=$docid?>,<?=$launchid?>,<?=$win_width?>,<?=$win_height?>,<?=$win_resize?>,<?=$win_scroll?>,<?=$win_directory?>,<?=$win_location?>,<?=$win_menu?>,<?=$win_tool?>,<?=$win_status?>);"><?=$launchfile?></a></td>
